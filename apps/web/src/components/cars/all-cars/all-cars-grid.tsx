...
/**
 * Virtualized, BIDIRECTIONAL infinite-scroll grid for the catalog — LOAD-ON-DEMAND
 * windowed architecture.
 *
 * The grid reserves scroll space only for what is LOADED (the contiguous window of
 * cars) plus one teaser page of skeletons below — NOT for the whole feed. Every
 * row sits at a fixed arithmetic position (uniform card heights make this exact —
 * see AuctionCard's deterministic-height contract); the loaded window occupies its
 * slice (`startCar` = the window's first car's index, seeded server-side as
 * `aboveCount` for `?after=` deep links), the teaser page below and any reserved
 * rows above a deep-link window render in-place `SkeletonCard`s. The document
 * height therefore grows a page at a time as you scroll and never approaches the
 * browser's element-height cap — see the reserved-space block for why bounding it
 * (vs reserving the full feed) is what fixes the height clamp, the stuck-at-bottom
 * wall and the drag-to-bottom request-spam. The deep tail is reached by FILTERING.
 *
...
export function AllCarsGrid({
  ...
  // Reserved space — LOAD ON DEMAND. We reserve rows only for what's LOADED plus
  // one teaser page of skeletons below (hinting "more coming"), NOT for the whole
  // feed. So the document height grows a page at a time as you scroll and never
  // approaches the browser's element-height cap: a ~1M-car feed at 735px/row would
  // want ~700M px, but reserving the whole thing is what created the ~30M-px clamp
  // (→ 87% of the feed unreachable + a dead "stuck" wall) AND the drag-to-bottom
  // request-spam (the scrollbar reached unloaded depths instantly and the loaders
  // walked there page by page). Bounding the reserve to the loaded window kills
  // both: the scrollbar can only span what you've loaded, so there is nothing to
  // walk and no wall to hit — the deep tail is reached by FILTERING, not scrolling.
  //
  // The upward reserve (`startCar` skeleton rows above the window) still exists for
  // `?after=` deep links, but since depth is now only reachable by scrolling, every
  // pointer is written shallow, so that reserve stays small too. The MAX_SIZER_PX
  // clamp is kept purely as a safety net against a pathologically deep stale link.
  const loadedEnd = startCar + cars.length;
  const totalCells = bottomDone ? loadedEnd : loadedEnd + CARS_PAGE_SIZE;
