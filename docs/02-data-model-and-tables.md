...
| `thumbnail_url` | text | | Per-source ~500px **card image URL served directly from the source CDN** (0035, column repurposed). Populated at ingestion by `cardImageUrl()` (copart `images.small[0]`; iaai `images.normal[0]` resized to 500×375; else `image_url`). Effectively never NULL — the catalog renders it in a plain `<img>` (no baking, no Vercel optimizer). |
| `thumbnail_source_url` | text | | **Vestigial** (0035). Was the change-detection key for the now-removed thumbnail-bake pipeline; no longer written or read (kept only because migrations are immutable). |
...
| display | `title`, `engine`, `image_url`, `thumbnail_url` (projected per-source card image URL served straight from the source CDN — effectively never NULL), `odometer_km`, `sale_date`, `status`, `condition`, `damage_main`, `seller`, `transmission`, `buy_now_price`, `bid_price`, `final_bid` |
