...
### 5.2 Backfilling card image URLs

Ingestion populates `auction_lots.thumbnail_url` (the per-source ~500px card image
URL served straight from the source CDN) on every upsert via `cardImageUrl()`. For
lots ingested **before** that logic existed, run the one-time
[`backfill-card-images.mjs`](../packages/db/backfill-card-images.mjs) — it
keyset-walks `auction_lots WHERE image_url IS NOT NULL` and computes the per-source
URL server-side (a SQL `CASE` mirroring `cardImageUrl()`, so `raw_json` never
leaves Neon), writing it into `thumbnail_url`. **No queue, no bake, no AWS creds** —
it's a pure DB job, safe against prod (idempotent, resumable, write-isolated).

```powershell
# from packages/db/
node --env-file-if-exists=../../.env backfill-card-images.mjs               # full run
node --env-file-if-exists=../../.env backfill-card-images.mjs --start=500000 --batch=20000 --sleep=50  # resume / tune
```

Then **reseed the projections** so `car_listings.thumbnail_url` picks up the new
values (the backfill only touches `auction_lots`):
...
The catalog resolves the card image as `thumbnailUrl ?? imageUrl`, so a lot missing
...
| A car card requests `/_next/image` | should never happen — car cards render a plain source-CDN `<img>`, never the Vercel optimizer | Investigate the web card: only the LOCAL logo/brand-logo assets use `next/image`. If a car card hits `/_next/image`, the mapper/card regressed. |
| A card image is missing / broken for some lots | `thumbnail_url` not populated for lots ingested before `cardImageUrl()` existed | Run the card-image backfill (§5.2), then reseed the projections. The card falls back to `image_url` meanwhile (`thumbnailUrl ?? imageUrl`). |
...
| Change the per-source card image URL logic | `cardImageUrl()` in `packages/functions/shared/normalize.ts` (+ the matching SQL `CASE` in `backfill-card-images.mjs`) → `pnpm run deploy` → re-run the card-image backfill (§5.2) so existing lots update |
